...
#endif
// Enable this module to use one of the following sensors: SENSOR_CHIRP
#ifndef MODULE_CHIRP
  #define MODULE_CHIRP 0
#endif
...
class SensorCHIRP: public Sensor {
  public:
    SensorCHIRP(NodeManager* node_manager, int child_id, I2CSoilMoistureSensor* chirp, int sensor_type);
    // define what to do at each stage of the sketch
    void onBefore();
    void onSetup();
    void onLoop();
    ...
    // constants
    const static int CAPACITANCE = 0;
    const static int TEMPERATURE = 1;
    const static int LIGHT = 2;
  protected:
    I2CSoilMoistureSensor* _chirp;
    int _sensor_type = 0;
    static int _chirp_address;
    static int _chirp_version;
    static int _chirp_moistureoffset;
    static int _chirp_moisturerange;
    static bool _chirp_moisturenormalized;
    static bool _chirp_lightreversed;
    int _chirp_capacitance = 0;
    int _chirp_temperature = 0;
    int _chirp_light = 0;
};
#endif
