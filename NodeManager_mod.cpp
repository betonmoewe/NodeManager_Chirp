...
int SensorCHIRP::_chirp_address = 0;
int SensorCHIRP::_chirp_version = 0;
int SensorCHIRP::_chirp_moistureoffset = 0;
int SensorCHIRP::_chirp_moisturerange = 0;
bool SensorCHIRP::_chirp_moisturenormalized = true;  
bool SensorCHIRP::_chirp_lightreversed = true;
// contructor
SensorCHIRP::SensorCHIRP(NodeManager* node_manager, int child_id, I2CSoilMoistureSensor* chirp, int sensor_type): Sensor(node_manager, child_id,A2) {
  _sensor_type = sensor_type;
  _chirp = chirp;
  ...
  else if (_sensor_type == SensorCHIRP::TEMPERATURE) {
    // humidity sensor
    setPresentation(S_TEMP);
    setType(V_TEMP);
    setValueType(TYPE_FLOAT);
  }
  else if (_sensor_type == SensorCHIRP::LIGHT) {
    // pressure sensor
    setPresentation(S_LIGHT_LEVEL);
    setType(V_LIGHT_LEVEL);
    setValueType(TYPE_FLOAT);
...
//void SensorCHIRP::setAddress(int value) {
//  _chirp_address = value;
//}
...
void SensorCHIRP::onSetup() {
  if (!_chirp_address) {
    _chirp->begin();
    delay(1000);
    _chirp_address = _chirp->getAddress();
    _chirp_version = _chirp->getVersion();
    //   if (_chirp->begin()) {
    //  }
    //  else {
    //    Serial.println(F("CHIRP offline"));
    //  }
    #if DEBUG == 1
      Serial.print("I2C Soil Moisture Sensor Address: ");
      Serial.println(_chirp_address,HEX);
      Serial.print("Sensor Firmware version: ");
      Serial.println(_chirp_version,HEX);
      Serial.println();
    #endif
  }
}
...
void SensorCHIRP::onLoop() {
  ...
  if (_sensor_type == SensorCHIRP::CAPACITANCE) {
    ...
    #if DEBUG == 1
      Serial.print(F("CHIRP I="));
      Serial.print(_child_id);
      Serial.print(F(" H="));
      Serial.print(capacitance);
      Serial.print(F(" H_orig="));
      Serial.print(capacitance_orig);     
      Serial.print(F(" Offset="));
      Serial.print(_chirp_moistureoffset);
      Serial.print(F(" Range="));
      Serial.print(_chirp_moisturerange);     
      Serial.print(F(" Normalized="));
      Serial.println(_chirp_moisturenormalized);       
    #endif
    // store the value
    _value_float = capacitance;
  }
  // temperature sensor
  else if (_sensor_type == SensorCHIRP::TEMPERATURE) {
    // read the temperature
...
// what to do when receiving a remote message
void SensorCHIRP::onProcess(Request & request) {
  int function = request.getFunction();
  switch(function) {
...
int NodeManager::registerSensor(int sensor_type, int pin, int child_id) {
  ...
  #endif
  #if MODULE_CHIRP == 1
    else if (sensor_type == SENSOR_CHIRP) {
      I2CSoilMoistureSensor* chirp = new I2CSoilMoistureSensor();
      // register SoilMoisture sensor
      registerSensor(new SensorCHIRP(this,child_id,chirp,SensorCHIRP::CAPACITANCE));
      // register temperature sensor
      child_id = _getAvailableChildId();
      registerSensor(new SensorCHIRP(this,child_id,chirp,SensorCHIRP::TEMPERATURE));
      // register light sensor
      child_id = _getAvailableChildId();
      return registerSensor(new SensorCHIRP(this,child_id,chirp,SensorCHIRP::LIGHT));
    }
  #endif
  else {
    #if DEBUG == 1
